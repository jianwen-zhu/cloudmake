...
def copy_sample_project(destination: Path) -> None:
    ...
    shutil.copy2(fixture / "Makefile", destination / "Makefile.build")
    shutil.copy2(fixture / "src" / "main.c", destination / "src" / "main.c")
...
def source_archive() -> bytes:
    ...
    with tarfile.open(fileobj=memory, mode="w:gz") as archive:
        ...
        archive.add(
            PROJECT_ROOT / "tests" / "fixtures" / "hello" / "src",
            arcname="src",
        )
...
def test_notebooks_expose_the_same_target_contract_without_credentials() -> None:
    for path, backend in (
        ...
        if path == COLAB_NOTEBOOK:
            ...
            assert "remote.py" in serialized

...
2. Decide whether an existing `artifacts/` or `.artifacts/` directory is project
   source. Cloudmake will synchronize and fingerprint it unless the project
   explicitly excludes it in `.cloudmakeignore`.
...
4. Run `cloudmake --sync-dry-run` and review the selected paths before any
   provider operation.
...
The sixth step matters for projects that may roll back to an older Cloudmake
release. This candidate excludes `.cloudmake/` automatically, but older releases
do not. Without an explicit `.cloudmakeignore` entry, rollback can select
collected output for remote upload. Without the `.gitignore` entry, it can also
...
Rollback does not move collected output back to `artifacts/`. Reconcile files
manually only after reviewing their contents and deciding which directory the
project owns.
...
does not establish legacy identity.

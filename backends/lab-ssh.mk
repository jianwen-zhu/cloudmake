# User-managed lab hosts expose an existing OpenSSH execution surface.
# Cloudmake uses the named host exactly as configured by the user and never
# provisions, reconfigures, starts, or stops the machine.
...
BACKEND_REQUIRED_VARIABLES := LAB_HOST
BACKEND_REQUIRES_PYTHON := yes
BACKEND_INSTALL_HINT := Install OpenSSH and rsync; configure LAB_HOST as an alias in ~/.ssh/config; then verify: ssh LAB_HOST true
BACKEND_VALIDATE := $(PYTHON_BIN) '$(CLOUDMAKE_TOOL_ROOT)/tools/validate_lab_host.py'
BACKEND_DOCTOR_PROBE := $(SSH_BIN) $(SSH_OPTIONS) -o BatchMode=yes '$(LAB_HOST)' true
BACKEND_VERSION_COMMAND := $(SSH_BIN) -V
BACKEND_TESTED_CLIENT := OpenSSH-compatible protocol 2 client
BACKEND_RESOURCE_ID := $(LAB_HOST)
...
BACKEND_START := :
BACKEND_STATUS = $(SSH_BIN) $(SSH_OPTIONS) -o BatchMode=yes '$(LAB_HOST)' "printf 'running\n'"
BACKEND_STOP = printf '%s\n' '[lab] LAB_HOST is user-managed; cloudmake will not stop or reconfigure it.'

.PHONY: refresh-ssh-config
refresh-ssh-config:
	@echo '[lab] OpenSSH configuration is user-managed; retrying without modification.' >&2
